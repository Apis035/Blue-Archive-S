...
//Abydos Foreclosure Task Force
    ...
    cex(a, 0) cpf(a, pf_shake) tfi() foc()
...
//Outside
    ...
    cmv(h1, 25) cmv(h3, 75) foc()
    cfi(h1, 3) cfi(h2, 3) cfi(h3, 3) wai(1.5)
    cpf(h1, pf_jump) tfi() foc()
        say(h1, "Hahaha!")
    cem(h2, em_exclaim)
        say(h2, "They're all out of ammo! Finish 'em off, and then take over the school!")
    cpf(h1, pf_shake) cpf(h2, pf_shake) cpf(h3, pf_shake) sfx("SE Rapid 01") foc()
        say("*pop* *pop* *pop* *tat*")
...
//Abydos Foreclosure Task Force
    ...
    cmv(a, 25) cex(a, 2) cfi(a, 3) cem(a, em_surprise) tfi() foc()
